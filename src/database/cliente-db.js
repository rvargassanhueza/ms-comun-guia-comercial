...
async function getCliente(){
    ...
    let query = 'SELECT * FROM T_CLIENTE WHERE vigente = 0'
...
async function getClienteId(id){
    let query = 'SELECT * FROM T_CLIENTE WHERE ID_CLIENTE = '+id+' AND vigente = 0';  
    const result = await pool.query(query);
...
async function deleteCliente(id){
    ...
    let query = 'UPDATE T_CLIENTE SET fecha_modificacion = ?,  usuario_modificacion = ?, vigente = ? WHERE ID_CLIENTE = '+id+'';
